...
A função discolect, definida como \textbf{discollect = lstr•id} no mónade das listas, \textbf{T A = [A]}, temos assim de seguida a função definida em haskell:
...
A função \textbf{dic\_exp} têm como prepósito exportar dicionários para o formato “lista de pares palavra-tradução”. Sendo \textbf{Dict = Exp String String}, \textbf{dic\_exp} transforma uma estrutura \textbf{Dict} para \textbf{[(String,[String])]}, onde a função \textbf{tar} cria uma lista de pares (palavra-tradução), e a função collect (de Exp.hs) agrupa as traduções que têm a mesma palavra.

\begin{code}
dic_exp :: Dict -> [(String,[String])]
dic_exp = collect.tar

tar = cataExp (either f g)
    where
      f x = [("",x)]
      g (h, l) = map (\(a,b) -> ((h++a),b)) (concat l)

\end{code}


A função \textbf{dic\_rd} têm como objetivo procurar traduções num dicionário para uma determinada palavra.
\textbf{dic\_rd} é um hylomorfismo composto por: um anamorfismo \textbf{searchRd}, que é a função que procura e verifica se a palavra existe no dicionário; e pelo catamorfismo \textbf{connectLeaves} que é responsável por agrupar as folhas (traduções).
Esta funcão devolve um \textbf{Maybe [String]}, onde dará Nothing se não existir, ou as respetivas traduções.

\begin{code}
dic_rd :: String -> Dict -> Maybe [String]
dic_rd = curry (transformMaybe . hyloExp connectLeaves searchRd)

searchRd :: (String, Dict) -> Either (Maybe String) (String, [(String, Dict)])
searchRd ("", Var v) = Left (Just v)
searchRd (t, Term o l) = if a == o then Right (a, map(\x -> (b,x)) l) 
                                    else Left Nothing
  where
    (a,b) = splitAt (length o) t
searchRd _ = Left Nothing

connectLeaves :: Either (Maybe String) (String, [[Maybe String]]) -> [Maybe String]
connectLeaves = g
  where g = (either f g)
            where
              f x = [x]
...
Por outro lado, temos agora \textbf{dic\_in} que têm como propósito inserir palavras novas (palavra e tradução) no dicionário.
Esta função é um hylomorfismo composto por: um anamorfismo \textbf{searchIn}, função que procura se a palavra existe no dicionário, onde caso exista, é adicionado a tradução, caso não exista é adicionado um novo termo; e por um catamorfismo \textbf{reBuild} que têm como objetivo reconstruir o dicionário.

\begin{code}
dic_in :: String -> String -> Dict -> Dict
dic_in = newCurry (hyloExp reBuild searchIn)
  where
...
searchIn (x, t, (Term o l))
    | x == "" = h
    | a == o = if b == "" then h
                            else Right (a, map (\x -> (b, t, x)) l)
    | otherwise = Left (Term o l)
    where
      (a, b) = splitAt (length o) x
      h = Left  (Term o (Var t: l))
searchIn (_, t, v) = Left v

reBuild :: Either Dict (String, [Dict]) -> Dict
reBuild = either id (uncurry Term)
\end{code}


Aqui estão descritas as funções auxiliares usadas na criação destes programas.
A função newCurry foi criada para ser possivel fazer curry de 3 argumentos;
A função auxiliar concatMaybes foi cosntruida para criar uma lista de valores Just de uma lista de Maybes, para que a função transformMaybe possa converter [Maybe String] para Maybe [String].

\begin{code}
newCurry :: ((a, b, c) -> d) -> a -> b -> c -> d
newCurry f a b c = f (a, b, c)

concatMaybes :: [Maybe a] -> [a]
concatMaybes [] = []
concatMaybes (h:t) = case h of
    Nothing -> concatMaybes t
    (Just x)->x:(concatMaybes t)

transformMaybe ::  [Maybe String] -> Maybe [String]
transformMaybe l = case (concatMaybes l)of [] -> Nothing
                                           l -> Just l
...
dir_aux :: (a,(Maybe a, Maybe a)) -> Maybe a
dir_aux (a, (l, Nothing)) = Just a
dir_aux (a,(l, r)) = r

maisEsq = cataBTree g
  where g = either (const Nothing) (esq_aux)

esq_aux :: (a,(Maybe a, Maybe a)) -> Maybe a
esq_aux (a, (Nothing, r)) = Just a
esq_aux (a,(l, r)) = l
...
insOrd' x = cataBTree g where g = either (const (Node(x, (Empty, Empty)), Empty)) (ins_aux) 
                                                                                           where ins_aux (a, ((esq1, esq2), (dir1, dir2))) = if (x < a) 
                                                                                                       then (Node(a, (esq1, dir2)), Node(a, (esq2, dir2)))
                                                                                                       else (Node(a, (esq2, dir1)), Node(a, (esq2, dir2)))

insOrd x  = p1 . insOrd' x

isOrd' = cataBTree g
  where g = either (const (True,Empty)) (ordena)

ordena :: (Ord a) => (a, ((Bool, BTree a), (Bool, BTree a))) -> (Bool, BTree a)
ordena (z, ((a, Empty), (b, Empty))) = (True, Node(z, (Empty, Empty)))
ordena (z, ((a, Empty), (b, x))) = if (cabeca x) < z 
                                                    then (False, Node(z, (Empty, x))) 
                                                    else (True, Node(z, (Empty, x)))
ordena (z, ((a, y), (b, Empty))) = if (cabeca y) > z 
                                                    then (False, Node(z, (y, Empty))) 
                                                    else (True, Node(z, (y, Empty)))
ordena (z, ((a, y), (b, x))) | (cabeca y) > z || (cabeca x) < z = (False, Node(z, (y, x)))
                             | otherwise = ((a && b), Node(z, (y, x)))
...
rrot Empty = Empty
rrot bt = bt
rrot (Node(r,(Node(r2,(dd,e)),ee))) = Node (r2,((dd),(Node(r,(e,ee)))))



lrot Empty = Empty
lrot bt = bt
lrot (Node(r,(d,(Node(r2,(dd,e)))))) = Node (r2,((Node(r,(e,dd))),e))

\end{code}


Recebendo uma [Bool] representa o caminho ao longo de uma árvore e seguindo pela esquerda, equivale ao valor "True" e seguindo pela esquerda, equivale ao valor "False".
O caminho ao longo de uma árvore serve para identificar unicamente um nó dessa árvore. 
\begin{code}

splay = (flip(cataBTree g)) where
        g = either (\x->(const Empty))  (curry t) 
        t ((a,(esq,dir)),[]) = Node (a,(esq [], dir []))
        t ((a,(esq,dir)),(h:t)) = if h
                                 then esq t
                                 else dir t
...
\subsection*{Diagrama do anamorfismo da Bdt:}
\begin{eqnarray*}
\xymatrix@@C=3cm{
    |Bdt|
&
...
navLTree = cataLTree g 
               where g = either (flip (const Leaf)) (curry aux1)

aux1 ((esq, dir),[]) = Fork(esq [], dir [])
aux1 ((esq, dir), (h:t)) = if h
                  then esq t
                  else dir t
...
bnavLTree = cataLTree g
                where g = either (flip(const Leaf)) (curry aux2)

aux2 ((a, as), Empty) = Fork(a Empty, as Empty)
aux2 ((a, as), Node(x, (Empty, t))) = if x
                                      then a Empty
                                      else as Empty
aux2 ((a, as), Node(x, (y, t))) = if x  
                                  then a y
                                  else as y
